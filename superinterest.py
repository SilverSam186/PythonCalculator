...
if simple_interest is None:
    ...
    if isinstance(simple_interest, float) and isinstance(rate, float) and isinstance(time, float):
        principal = simple_interest / (rate * time)
        print("Principal:", format_money(principal, currency))

elif rate is None:
    if isinstance(simple_interest, float) and isinstance(principal, float) and isinstance(time, float):
        rate = simple_interest / (principal * time)
        print("Rate Percent:", f"{rate * 100:.2f}%")

elif time is None:
    if isinstance(simple_interest, float) and isinstance(principal, float) and isinstance(rate, float):
